...
struct dynarray* dynarray_create() {
  struct dynarray* dynamic=malloc(sizeof(struct dynarray));
  dynamic->capacity = 2;
  dynamic ->size = 0;
  dynamic->data=malloc(sizeof(void*)*dynamic->capacity);
  return dynamic;
}
...
void dynarray_insert(struct dynarray* da, void* val) {
    //when array is at capacity
    if(da->size == da->capacity){
      da->capacity = da->capacity * 2; //double size of the array
      da->data = realloc(da->data,da->capacity * sizeof(void*));   
    } 
    da->data[da->size] = val; //size is the index + 1. This lets the new size be used to add a new element at the end.
    da->size++; 
...
void dynarray_remove(struct dynarray* da, int idx) {
  ...
  // }
    ...
    if(da->size > 0 && idx < da->size && idx >= 0) { // Ensure valid index
    for(int i = idx; i < da->size - 1; i++) { // Loop from idx to size - 1
      da->data[i] = da->data[i + 1];
    }
    da->size--;
  }
