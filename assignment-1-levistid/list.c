...
struct list* list_create() {
  struct list* lista= malloc(sizeof(struct list)); //allocate memory   
  lista->head = NULL; 
  return lista;
}
...
void list_free(struct list* list) {
    if(list){
          struct link *currnet = list->head; 
    while(currnet){ 
      struct link * next = currnet->next;
     // currnet = list->head; 
      free(currnet); // Free the node
      currnet = next; // move to next node
    }
    free(list);
    }
}
...
struct link* new_node = malloc(sizeof(struct link));
  new_node->val = val;
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b)) {
      struct link* current =list->head, *prev = NULL;
      while(current){
...
void list_reverse(struct list* list) {
  struct link * previous = NULL, *current = list->head, *next = NULL;
  
  while(current != NULL){
   next = current->next; //space for next node
   current->next = previous; // points to previous
   previous = current; // changes previous to current
   current = next;  //current to next via next link
  }
  list->head = previous; // new head of list
  return;
}
